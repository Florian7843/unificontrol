...
from .json_fixers import (
    fix_note_noted,
    fix_user_object_nesting,
    fix_macs_list,
    fix_end_now,
    fix_start_12hours,
    fix_start_7days,
...
class UnifiClient(metaclass=MetaNameFixer):
    ...
    def login(self, username=None, password=None):
        ...
        """
        self._login(
            username=username if username else self._user,
            password=password if password else self._password,
        )

    logout = UnifiAPICallNoSite(
        "Log out from Unifi controller", "logout", need_login=False
    )
    ...
    create_client = UnifiAPICall(
        ...
        """,
        "group/user",
        json_args=["mac", "usergroup_id", ("name", None), ("note", None)],
    ...
    stat_5minutes_site = UnifiAPICall(
        ...
        json_args=[("start", None), ("end", None), ("attrs", _DEFAULT_SITE_ATTRIBUTES)],
        json_fix=[
            fix_end_now,
            fix_start_12hours,
            fix_ensure_time_attrib,
    ...
    stat_sessions = UnifiAPICall(
        "Show login sessions",
        "stat/session",
        json_args=[("mac", None), ("start", None), ("end", None), ("type", "all")],
        json_fix=[
            fix_end_now,
            fix_start_7days,
    ...
    stat_sta_sessions_latest = UnifiAPICall(
        ...
        "stat/session",
        json_args=["mac", ("limit", 5)],
        json_fix=[
            fix_constants({"_sort": "-assoc_time"}),
    ...
    stat_auths = UnifiAPICall(
        ...
        "stat/authorization",
        json_args=[("start", None), ("end", None)],
        json_fix=[fix_end_now, fix_start_7days],
    )

    list_allusers = UnifiAPICall(
        "List all client devices ever connected to the site",
        "stat/alluser",
        json_args=[("type", "all"), ("conn", "all"), ("within", 365 * 24)],
    )
    ...
    create_usergroup = UnifiAPICall(
        "Create user group",
        "rest/usergroup",
        json_args=["name", ("qos_rate_max_down", -1), ("qos_rate_max_up", -1)],
    )
    ...
    create_admin = UnifiAPICall(
        ...
        json_fix=[
            fix_arg_names({"enable_sso": "for_sso", "password": "x_password"}),
            fix_admin_permissions,
            fix_check_email("email"),
        ],
    )
    ...
    create_hotspotop = UnifiAPICall(
        ...
        "rest/hotspotop",
        json_args=["name", "password", "note"],
        json_fix=[fix_note_noted, X_PASSWORD_FIX],
    )
    ...
    list_events = UnifiAPICall(
        ...
        """,
        "stat/event",
        json_args=[("historyhours", 720), ("start", 0), ("limit", 1000)],
        json_fix=[
            fix_arg_names(
                {"historyhours": "within", "start": "_start", "limit": "_limit"}
            ),
            fix_constants({"_sort": "-time", "type": None}),
        ],
    )
